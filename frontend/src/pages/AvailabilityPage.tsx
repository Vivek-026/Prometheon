    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <CardContent className="pt-6">
             <div className="flex items-center gap-10">
                <div className="space-y-2 max-w-sm">
                   <Label className="text-lg font-black uppercase italic text-white flex items-center gap-2">
                       <Clock className="text-[#F97316]" size={20} /> Base Offline Quota
                   </Label>
                   <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest leading-relaxed">
                      Daily non-work baseline including sleep, maintenance, and essential chores. 
                      Standard allocation: 11 hours.
                   </p>
                </div>
                <div className="flex items-center gap-4">
                   <Input 
                     ...
                     className="w-24 h-14 bg-[#111] border-[#2e2e2e] rounded-none text-2xl font-black text-center text-[#F97316] outline-none focus-visible:border-[#F97316]"
                   />
                   <span className="text-xl font-black uppercase italic text-zinc-700 tracking-tighter">HOURS / DAY</span>
                </div>
             </div>
          </CardContent>
       </Card>
          ...
          {Array.isArray(weeklySlots) && weeklySlots.map(slot => (
             <Card key={slot.day} className={cn("bg-[#1a1a1a] border rounded-none transition-all", getBandBg(slot.band))}>
                <div className="p-3 border-b border-inherit flex flex-col items-center gap-1">
                   <span className="text-[11px] font-black uppercase tracking-tighter text-zinc-400">{slot.day}</span>
                   <Badge className={cn("text-[9px] font-black h-5", getBandColor(slot.band))}>{slot.band}</Badge>
                </div>
                <div className="p-3 space-y-4">
                   {/* Slots List */}
                   <div className="space-y-2 min-h-[100px]">
                      {!Array.isArray(slot.blocked_slots) || slot.blocked_slots.length === 0 ? (
                         <p className="text-[8px] text-zinc-700 font-bold uppercase italic text-center py-4">No blockages reported</p>
                      ) : (
                         slot.blocked_slots.map((block, idx) => (
                            <div key={idx} className="group relative bg-[#0d0d0d]/80 border border-[#2e2e2e] p-2 flex flex-col gap-1 transition-all hover:border-[#F97316]/40">
                               <div className="flex items-center justify-between">
                                  <span className="text-[9px] font-black text-[#F97316] italic">{block.start_time} - {block.end_time}</span>
                               ...
                               <span className="text-[8px] font-black uppercase truncate text-zinc-400">{block.reason_category}</span>
                            </div>
                         ))
                      )}
                   </div>
                   ...
                   {addingToDay === slot.day ? (
                      <div className="bg-[#111] p-2 border border-[#F97316]/50 space-y-2 animate-in zoom-in-95 duration-200">
                         <div className="grid grid-cols-2 gap-1">
                            <Input 
                              ...
                              className="h-6 text-[9px] p-1 bg-[#0d0d0d] border-[#2e2e2e] rounded-none"
                            />
                            <Input 
                              type="time" 
                         ...
                         <select 
                           ...
                           className="w-full h-6 text-[9px] bg-[#0d0d0d] border-[#2e2e2e] text-zinc-400 outline-none p-0.5"
                         >
                         ...
                         <Input 
                           ...
                           className="h-6 text-[8px] p-1 bg-[#0d0d0d] border-[#2e2e2e] rounded-none"
                         />
                         <div className="flex gap-1 pt-1">
                            <Button size="sm" onClick={() => handleAddSlot(slot.day)} className="flex-1 h-5 text-[8px] font-black bg-[#F97316] text-black">ADD</Button>
                            <Button size="sm" variant="outline" onClick={() => setAddingToDay(null)} className="h-5 text-[8px] font-black border-[#2e2e2e]">X</Button>
                         </div>
                      </div>
                   ) : (
                      <button 
                      ...
                      >
                         <Plus size={12} /> <span className="text-[9px] font-black uppercase tracking-widest">Add Block</span>
                      </button>
                   )}

                   <div className="pt-2 border-t border-inherit flex justify-between items-center">
                      <span className="text-[8px] font-black uppercase text-zinc-500">AVAILABLE</span>
                      <span className="text-[10px] font-black italic text-zinc-300">{slot.available_hours.toFixed(1)}H</span>
                   </div>
                </div>
             </Card>
          ))}
       ...
       <div className="flex justify-end pt-4">
           <Button 
             ...
             disabled={isPending}
             className="h-12 px-12 bg-[#F97316] hover:bg-[#F97316]/90 text-black font-black uppercase tracking-[0.3em] rounded-none shadow-[0_0_20px_rgba(249,115,22,0.2)] flex gap-3"
           >
...
const AvailabilityPage: React.FC = () => {
    ...
    const saveMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            alert('Availability synchronized with command center.');
        }
    });
    ...
    return (
        <div className="flex bg-[#111111] min-h-screen font-mono text-zinc-300">
            ...
            <main className="flex-1 ml-64 p-10 flex flex-col h-screen overflow-hidden">
                <header className="flex items-center justify-between mb-10 shrink-0">
                    <div className="flex items-center gap-6">
                        <div className="w-12 h-12 bg-[#F97316] flex items-center justify-center italic font-black text-black text-2xl shadow-[0_0_20px_rgba(249,115,22,0.3)]">
                           ⏳
                        </div>
                        <div>
                           <h1 className="text-3xl font-black uppercase tracking-tighter italic text-white">Temporal Registry</h1>
                           <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-[0.3em] mt-1 italic">Operative Availability & Load Matrix</p>
                        </div>
                    </div>
                </header>
                ...
                <div className="mb-8 shrink-0 flex items-center justify-between">
                   <div className="bg-[#1a1a1a] p-1 border border-[#2e2e2e] flex gap-1">
                      <button 
                        ...
                        className={cn(
                          "py-2 px-8 text-[10px] font-black uppercase transition-all rounded-none",
                                ...
                                <table className="w-full text-left border-collapse">
                                   <thead>
                                      <tr className="border-b border-[#2e2e2e] bg-[#111]">
                                         <th className="p-4 text-[10px] font-black uppercase tracking-widest text-zinc-500">OPERATIVE</th>
                                         {DAYS.map(day => (
                                           <th key={day} className="p-4 text-[10px] font-black uppercase tracking-widest text-zinc-500 text-center w-28">{day.slice(0,3)}</th>
                                         ))}
                                         <th className="p-4 text-[10px] font-black uppercase tracking-widest text-zinc-500 text-right">LATEST UPLINK</th>
                                      </tr>
                                   </thead>
                                   <tbody>
                                      {isLoadingTeam ? (
                                          ...
                                          >
                                             <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                   ...
                                                   <div>
                                                      <p className="text-xs font-black uppercase text-white">{member.user.name}</p>
                                                      {isStale && <span className="text-[8px] text-red-500 font-bold uppercase italic flex items-center gap-1"><AlertTriangle size={8} /> stale records</span>}
                                                   </div>
                                                </div>
                                             </td>
                                             {/* 
                                               ...
                                               <td key={day} className="p-4">
                                                  <div className={cn(
                                                    ...
                                                    getBandColor(member.band) // Simplifying: team grid shows current/avg band
                                                     ...
                                                     <span className="text-[8px] font-black">{member.band}</span>
                                                  </div>
                                               </td>
                                             ))}
                                             <td className="p-4 text-right">
                                                <span className="text-[9px] font-bold text-zinc-600 uppercase italic whitespace-nowrap">
            ...
            <Dialog open={!!viewingDevId} onOpenChange={() => setViewingDevId(null)}>
               <DialogContent className="max-w-5xl bg-[#0d0d0d] border-[#2e2e2e] text-zinc-300 font-mono flex flex-col p-0 max-h-[90vh]">
                  <DialogHeader className="p-8 border-b border-[#2e2e2e]">
                     <DialogTitle className="text-2xl font-black uppercase italic text-[#F97316] flex items-center gap-3">
                        <Lock size={24} /> Operative Profile Security Review
                     </DialogTitle>
                     <DialogDescription className="text-xs text-zinc-500 uppercase tracking-widest">
                        Analyzing Temporal Distribution for: <span className="text-white">{devDetail?.user_id}</span>
                     </DialogDescription>
                  </DialogHeader>
                       ...
                       <div className="space-y-10">
                          <div className="flex items-center gap-8">
                             <div className="p-6 bg-zinc-900 border border-[#2e2e2e]">
                               <p className="text-[10px] font-black text-zinc-500 uppercase mb-2">Base Inactivity Quota</p>
                               <p className="text-4xl font-black italic">{devDetail?.base_unavailable_hours}h</p>
                             </div>
                             <div className="flex-1 space-y-2">
                                <p className="text-[10px] font-black text-zinc-500 uppercase">Last Uplink Signal</p>
                                <p className="text-xs uppercase font-bold">{devDetail?.updated_at && new Date(devDetail.updated_at).toLocaleString()}</p>
                                <Separator className="bg-[#2e2e2e]" />
                                <p className="text-[9px] text-zinc-600 leading-relaxed uppercase tracking-tighter">Verified by Command Center Security Protocol.</p>
                             </div>
                          </div>
                             ...
                             {devDetail?.weekly_slots.map(slot => (
                               <div key={slot.day} className={cn("border p-3 space-y-3", getBandBg(slot.band))}>
                                  <div className="pb-2 border-b border-inherit space-y-1">
                                     <p className="text-[10px] font-black uppercase text-center">{slot.day.slice(0,3)}</p>
                                     <Badge className={cn("text-[8px] font-black w-full justify-center h-4", getBandColor(slot.band))}>{slot.band}</Badge>
                                  </div>
                                  <div className="space-y-1.5 min-h-[80px]">
                                     {slot.blocked_slots.map((block, i) => (
                                       <div key={i} className="text-[8px] bg-[#000]/30 p-1.5 border border-inherit">
                                          <p className="font-black text-[#F97316]">{block.start_time}-{block.end_time}</p>
                                          <p className="uppercase text-zinc-500 truncate">{block.reason_category}</p>
                                       </div>
                                     ))}
                                  </div>
                                  <div className="pt-2 border-t border-inherit text-center">
                                     <p className="text-[9px] font-black italic">{slot.available_hours}H</p>
                                  </div>
                               </div>
                             ))}
                          </div>
                  ...
                  <DialogFooter className="p-6 bg-[#111] border-t border-[#2e2e2e]">
                     <Button onClick={() => setViewingDevId(null)} className="rounded-none bg-zinc-800 hover:bg-zinc-700 text-white font-black uppercase px-10">Termination</Button>
                  </DialogFooter>
               </DialogContent>
            </Dialog>
        </div>
