  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        />
        {unreadCount > 0 && (
          <div className="absolute top-1 right-1 h-4 w-4 bg-[#F97316] text-black text-xs font-medium flex items-center justify-center shadow-[0_0_8px_rgba(249,115,22,0.5)]">
            {displayCount}
          </div>
        )}
      </button>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 max-h-[500px] bg-[#0a0a0a] border border-[#2e2e2e] shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-[100] animate-in fade-in zoom-in-95 duration-200 overflow-hidden flex flex-col">
          <div className="p-3 border-b border-[#2e2e2e] flex items-center justify-between bg-[#0d0d0d]">
            <h3 className="text-xs font-medium text-zinc-500">Alert_Stream_Audit</h3>
            {unreadCount > 0 && (
              <button 
                onClick={markAllAsRead}
                className="text-xs font-medium text-[#F97316] hover:text-white transition-colors"
              >
                Mark_All_As_Read
              </button>
            )}
          ...
          <div className="overflow-y-auto max-h-[400px] custom-scrollbar bg-black/40 backdrop-blur-md">
            {isLoading && notifications.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center p-10 text-center space-y-3 opacity-20 grayscale grayscale-0">
                ...
                <span className="text-xs font-medium leading-tight">All Sector Protocols_Caught Up 🎉</span>
              </div>
            ) : (
          ...
          >
            <span className="text-xs font-medium text-zinc-600 group-hover:text-[#F97316]">Analyze_Full_Registry_Archive</span>
          </div>
        </div>
      )}
    </div>
